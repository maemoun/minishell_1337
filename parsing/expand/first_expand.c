...
/*   Updated: 2025/09/07 17:53:53 by abdo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	handle_heredoc(const char *input, t_fir_expand_elements *st)
{
		...
		int last_exit_status)
{
	...
	st = (t_fir_expand_elements){env, last_exit_status, 0, 0,
		0, ft_strndup("", 0)};
	while (input[st.i])
	{
		if (input[st.i] == '<' && input[st.i + 1] == '<')
		{
			handle_heredoc(input, &st);
			continue ;
		}
